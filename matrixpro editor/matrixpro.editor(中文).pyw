...
try:
    ...
    messagebox.showerror(
        message=
        'This editor requires modules "pillow" and "yapf" to work, just type "pip install pillow yapf" in cmd and press enter, and then reopen the editor will fix this error.\n这个编辑器需要python库"pillow"和"yapf"以正常运行，只要在cmd里输入"pip install pillow yapf"然后按下回车，然后再重新打开编辑器就可以了。'
    )
    sys.exit(1)
...
class Root(Tk):
    ...
    def __init__(self):
        ...
        self.realtime_box.place(x=420, y=0)
        self.auto_box.place(x=520, y=0)
